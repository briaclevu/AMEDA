...
% set the paths
path_in=[pwd,'/Tuto/DATA/',source,'/',domain,'/',runname,'/'];
path_result=[pwd,'/Tuto/RESULT/',source,'/',domain,'/',config,postname,'/'];
path_out=[path_result,'/',runname,'/'];
path_rossby=[pwd,'/Rossby_radius/'];
...
nc_u = [path_in,'ssu_',sshtype,'_',domain,'_',runname,postname,'.nc'];
nc_v = [path_in,'ssv_',sshtype,'_',domain,'_',runname,postname,'.nc'];
nc_ssh = [path_in,'../ssh_',sshtype,'_',domain,postname,'.nc'];
...
nRmin = 3/2; % half of the native Dx grid size

% variable names (could be automatised)
y_name = 'lat';
x_name = 'lon';
m_name = 'mask';
if strcmp(runname,'geo')
  u_name = 'ugeo';%'u'
  v_name = 'vgeo';%'v'
else
  u_name = 'u';%'u'
